...
Original file is located at
    https://colab.research.google.com/drive/1Fa95hA3OkK0i8DbHXSc1gHdRgSA5--jL
"""
...
workspace_dir = './hw2_data/digits/mnistm/'
...
def get_dataset(root, csvfile, transform):
    ...
    dataset = p2Data(fnames, labels, transform)
    return dataset


train_set = get_dataset(os.path.join(workspace_dir, 'data'), os.path.join(
    workspace_dir, 'train.csv'), train_tfm)
test_set = get_dataset(os.path.join(workspace_dir, 'data'), os.path.join(
    workspace_dir, 'val.csv'), test_tfm)
...
save_dir = 'output_diffusion_new'
save_img_dir = 'output_diffusion_img'
os.makedirs(save_dir, exist_ok=True)
os.makedirs(save_img_dir, exist_ok=True)

ddpm = DDPM(nn_model=ContextUnet(in_channels=3, n_feat=n_feat, n_classes=n_classes),
            betas=(1e-4, 0.02), n_T=n_T, device=device, drop_prob=0.1)
# ddpm.load_state_dict(torch.load('output_diffusionmodel_19.pth'))
...
for ep in range(n_epoch):
    ...
    if ep == 5 or ep >= 10:
        ...
        with torch.no_grad():
            ...
            for k in range(n_classes):
                x_gen, x_gen_store = ddpm.sample(
                    n_sample, (3, 28, 28), device, guide_w=0)
                for j in range(100):
                    oneImage = x_gen[j].to('cpu').moveaxis(
                        0, 2).clamp(0, 1).numpy()
                    c, idx = j % 10, k*10+(j//10)
                    # c = j % 10
                    plt.imsave(
                        f'{save_img_dir}/{str(c)}_{str(idx).zfill(3)}.png', oneImage)
                    print(f"saved image {str(c)}_{str(idx).zfill(3)}.png")

            # x_all = torch.cat([x_gen, x_real])
            save_image(x_gen, save_dir + "/" + f"x_gen_ep{ep}.png")
            print('saved image at ' + save_dir + "/" + f"x_gen_ep{ep}.png")

        print("=========================================================")
        print('Evaluating image accuarcy...')
        ...
        data_loader = torch.utils.data.DataLoader(DATA(save_img_dir),
                                                  batch_size=32,
                                                  num_workers=4,
                                                  shuffle=False)
        ...
        with torch.no_grad():
            for idx, (imgs, labels) in enumerate(data_loader):
                ...
                total += len(pred)
        print('acc = {} (correct/total = {}/{})'.format(float(correct) /
                                                        total, correct, total))
        acc = correct/total
        if acc > best_acc:
            best_acc = acc
            ...
            torch.save(ddpm.state_dict(), os.path.join(
                save_dir, f"model_best.pth"))
            print('saved model at ' + save_dir + "/" + f"model_best.pth")
        print(f'Best model saved at epoch {ep} with accuracy = {best_acc}')
