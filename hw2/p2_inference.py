...
# save_img_dir = './p2_output'
save_img_dir = sys.argv[1]

ddpm = DDPM(nn_model=ContextUnet(in_channels=3, n_feat=n_feat, n_classes=n_classes),
            betas=(1e-4, 0.02), n_T=n_T, device=device, drop_prob=0.1)
# ddpm.load_state_dict(torch.load('./output_diffusion_new/model_best.pth'))
ddpm.load_state_dict(torch.load('model_best.pth'))
ddpm.to(device)
...
with torch.no_grad():
    ...
    for k in range(n_classes):
        x_gen, x_gen_store = ddpm.sample(
            n_sample, (3, 28, 28), device, guide_w=1)
        for j in range(100):
            oneImage = x_gen[j].to('cpu').moveaxis(
                0, 2).clamp(0, 1).numpy()
            c, idx = j % 10, k*10+(j//10)+1
            # c = j % 10
            # plt.imsave(
            #     f'{save_img_dir}/{str(c)}_{str(idx).zfill(3)}.png', oneImage)
            plt.imsave(os.path.join(
                save_img_dir, f'{str(c)}_{str(idx).zfill(3)}.png'), oneImage)
