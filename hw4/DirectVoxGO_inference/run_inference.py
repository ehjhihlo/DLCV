...
def config_parser():
    ...
    parser.add_argument('--config', default='configs/nerf/hotdog.py',
                        help='config file path')
    parser.add_argument("--seed", type=int, default=777,
                        help='Random seed')
    parser.add_argument("--no_reload", action='store_true',
                        help='do not reload weights from saved ckpt', default=False)
    parser.add_argument("--no_reload_optimizer", action='store_true',
                        help='do not reload optimizer state from saved ckpt', default=False)
    ...
    parser.add_argument("--render_only", action='store_true',
                        help='do not optimize, reload weights and render out render_poses path', default=True)
    ...
    # hw4 inference
    # parser.add_argument("-input", "--input_dir", default='/home/tom/Documents/DLCV/hw4_data/hotdog/transforms_val.json', help='input json file', type=str)
    # parser.add_argument("-output", "--output_dir", default='/home/tom/Documents/DLCV/DirectVoxGO_2/output', help='output image folder', type=str)
    parser.add_argument("--input_dir", default='/home/tom/Documents/DLCV/hw4_data/hotdog/transforms_val.json', help='input json file', type=str)
    parser.add_argument("--output_dir", default='/home/tom/Documents/DLCV/DirectVoxGO_2/output', help='output image folder', type=str)
...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    if savedir is not None and dump_images:
        for i in trange(len(rgbs)):
            ...
            filename = os.path.join(savedir, f'r_{i}.png')
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    HW, Ks, near, far, i_test, poses, render_poses, images = [
    ...
    def gather_training_rays():
        if data_dict['irregular_shape']:
            rgb_tr_ori = [images[i].to('cpu' if cfg.data.load2gpu_on_the_fly else device) for i in i_test]
        else:
            rgb_tr_ori = images[i_test].to('cpu' if cfg.data.load2gpu_on_the_fly else device)

        if cfg_train.ray_sampler == 'in_maskcache':
            rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz = dvgo.get_training_rays_in_maskcache_sampling(
                    rgb_tr_ori=rgb_tr_ori,
                    train_poses=poses[i_test],
                    HW=HW[i_test], Ks=Ks[i_test],
                    ndc=cfg.data.ndc, inverse_y=cfg.data.inverse_y,
                    flip_x=cfg.data.flip_x, flip_y=cfg.data.flip_y,
            ...
            rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz = dvgo.get_training_rays_flatten(
                rgb_tr_ori=rgb_tr_ori,
                train_poses=poses[i_test],
                HW=HW[i_test], Ks=Ks[i_test], ndc=cfg.data.ndc, inverse_y=cfg.data.inverse_y,
                flip_x=cfg.data.flip_x, flip_y=cfg.data.flip_y)
        else:
...
if __name__=='__main__':
    ...
    # cfg.data['datadir'] = sys.argv[1]
    cfg.data['datadir'] = args.input_dir
    ...
    if args.render_test:
        if args.ft_path:
            ...
            ckpt_path = os.path.join('fine_last2.tar')
            # ckpt_path = os.path.join('DirectVoxGO_inference','fine_last2.tar')
    ...
    if args.render_test:
        # testsavedir = os.path.join(cfg.basedir, cfg.expname, f'render_test_{ckpt_name}')
        # testsavedir = sys.argv[2]
        testsavedir = args.output_dir
        os.makedirs(testsavedir, exist_ok=True)
        print('All results are dumped into', testsavedir)
        rgbs, depths, bgmaps = render_viewpoints(
                render_poses=data_dict['poses'][data_dict['i_test']],
