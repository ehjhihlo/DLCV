...
__global__ void sample_bg_pts_on_rays_cuda_kernel(
  ...
  if(idx<N_samples*n_rays) {
    ...
    # r = R * R / t
    o2i_p = R_outer.powf(2) / t_outer.powf(2) * (1-self.bg_preserve) + R_outer / t_outer * self.bg_preserve
...
__global__ void raw2alpha_cuda_kernel(
  ...
  if(i_pt<n_pts) {
    ...
    exp_d[i_pt] = e;
    alpha[i_pt] = 1 - powf(1 + e, -interval);
  }
}
...
__global__ void raw2alpha_nonuni_cuda_kernel(
  ...
  if(i_pt<n_pts) {
    const scalar_t e = exp(density[i_pt] + shift); // can be inf
    exp_d[i_pt] = e;
    alpha[i_pt] = 1 - powf(1 + e, -interval[i_pt]);
  }
}
...
__global__ void raw2alpha_backward_cuda_kernel(
  ...
  const int i_pt = blockIdx.x * blockDim.x + threadIdx.x;
  if(i_pt<n_pts) {
    grad[i_pt] = min(exp_d[i_pt], 1e10) * powf(1+exp_d[i_pt], -interval-1) * interval * grad_back[i_pt];
  }
}
...
__global__ void raw2alpha_nonuni_backward_cuda_kernel(
  ...
  const int i_pt = blockIdx.x * blockDim.x + threadIdx.x;
  if(i_pt<n_pts) {
    grad[i_pt] = min(exp_d[i_pt], 1e10) * powf(1+exp_d[i_pt], -interval[i_pt]-1) * interval[i_pt] * grad_back[i_pt];
  }
}
